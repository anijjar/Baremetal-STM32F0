...
void onboard_led_blue_init() {
    ...
    SET_BIT(GPIOC->ODR, onboard_blue_led_state << ONBOARD_BLUE_LED_PIN);
}
...
void onboard_led_green_init() {
    ...
    SET_BIT(GPIOC->ODR, onboard_green_led_state << ONBOARD_GREEN_LED_PIN);
}

void led_on(uint8_t led, GPIO_TypeDef * GPIOx){
    SET_BIT(GPIOx->ODR, 1 << led);
    if(led == ONBOARD_BLUE_LED_PIN){
        onboard_blue_led_state = LED_ON;
    } else if(led == ONBOARD_GREEN_LED_PIN){
        onboard_green_led_state = LED_ON;
    }
}
void led_off(uint8_t led, GPIO_TypeDef * GPIOx){
    CLEAR_BIT(GPIOx->ODR, 1 << led);
    if(led == ONBOARD_BLUE_LED_PIN){
        onboard_blue_led_state = LED_OFF;
    } else if(led == ONBOARD_GREEN_LED_PIN){
        onboard_green_led_state = LED_OFF;
    }
}
void led_toggle(uint8_t led, GPIO_TypeDef * GPIOx){
    TOGGLE_BIT(GPIOx->ODR, 1 << led);
    if(led == ONBOARD_BLUE_LED_PIN){
        onboard_blue_led_state = READ_REG(GPIOx->ODR) & (1 << led);
    } else if(led == ONBOARD_GREEN_LED_PIN){
        onboard_green_led_state = READ_REG(GPIOx->ODR) & (1 << led);
    }
}
