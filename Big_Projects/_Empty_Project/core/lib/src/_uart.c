...
void USARTx_INIT(eUsartPeriperal_t periph, sGPIO_t sTx, sGPIO_t sRx) {
    ...
    if(periph == USART1_PERIPHERAL){
        ...
        usartdiv =  CORE_CLOCK_HZ / USART_1_BAUD;
        #ifdef USART_1_OVER8
...
int USARTx_RECIEVE_MESSAGE(USART_TypeDef * USARTx, uint8_t * ret){
    for(int i = 0; i < USART_1_BUFFER_SIZE; i++){
        ret[i] =  USARTx_RECEIVE_CHAR(USART1);
        while((USARTx->ISR & USART_ISR_RXNE) != USART_ISR_RXNE);
        if(ret[i] == '\n' || ret[i] == '\r'){
            return 0;
        }
    } 
